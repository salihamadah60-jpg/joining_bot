...
- رسم بياني سطحي (AreaChart) لمعدل الانضمام اليومي والفشل وFLOOD_WAIT — آخر 14 يوم
- رسم دائري (PieChart) لتوزيع الأخطاء حسب كود الخطأ — آخر 30 يوم
- رسم بياني أفقي (BarChart) لأداء كل حساب (انضمامات + فشل)
- جدول تفصيلي بكل حساب: إجمالي، اليوم، معدل نجاح، عدد مجموعاته، الحالة
- بطاقات KPI: إجمالي الانضمامات، اليوم، معدل النجاح، حسابات نشطة، روابط معلقة، FLOOD في 24h
- **الملفات:** `artifacts/api-server/src/routes/analytics.ts`, `artifacts/dashboard/src/pages/Analytics.tsx`

### [P3-3] ✅ نسخ احتياطي للجلسات في MongoDB
- نسخ احتياطي يدوي لكل جلسات الحسابات إلى MongoDB بضغطة زر
- استعادة الجلسات من MongoDB عند فقدان القاعدة المحلية
- إعداد رابط MongoDB مخصص من صفحة الإعدادات
- **الملفات:** `artifacts/api-server/src/lib/mongoSessionBackup.ts`, `artifacts/api-server/src/routes/sessions.ts`, `artifacts/dashboard/src/pages/Settings.tsx`
